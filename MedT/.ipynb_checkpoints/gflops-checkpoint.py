...
def MedT(imgDim,channels,numclasses,pretrained=False, **kwargs):
    model = medt_net(AxialBlock_dynamic,AxialBlock_wopos, [1, 2, 4, 1], s= 0.125,num_classes=numclasses, imgchan=channels,img_size =imgDim,  **kwargs)
    return model
...
# ---------------------------------------------------------
# 1. Define Custom Handlers for Axial Attention layers
# ---------------------------------------------------------

def count_axial_attention_dynamic(m, x, y):
    """
    Custom FLOPs counter for AxialAttention_dynamic.
    Captures the 5 expensive einsum operations: qk, qr, kr, sv, sve
    """
    # x[0] is input tensor (N, W, C, H) or (N, C, H, W) depending on permute
    # But thop passes the module (m), input (x), output (y)
    
    inp = x[0]
    batch_size = inp.shape[0]
    
    # Based on the forward pass logic:
    # N, W, C, H logic is applied. 
    # If width=False: x.permute(0, 3, 1, 2) -> N, H, C, W. 
    # The Attention happens on the last dimension relative to groups.
    
    # We derive dimensions from the config of the layer
    groups = m.groups
    group_planes = m.group_planes
    
    # Current feature map spatial size (H or W depending on the axial direction)
    # The input to qkv_transform is (N*W, C, H) roughly speaking
    # Let's extract H (the sequence length for attention)
    if m.width:
        # Input was (N, C, H, W), permuted to (N, H, C, W)
        # Sequence length is W
        seq_len = inp.shape[3] 
        other_dim = inp.shape[2]
    else:
        # Input was (N, C, H, W), permuted to (N, W, C, H)
        # Sequence length is H
        seq_len = inp.shape[2]
        other_dim = inp.shape[3]

    # Effective Batch Size for the attention mechanism = Batch * Other_Spatial_Dim
    effective_batch = batch_size * other_dim
    
    # 1. qkv_transform (Conv1d)
    # This is a standard Conv module inside the class. 
    # thop counts sub-modules automatically! 
    # We ONLY need to count the einsums here.
    
    # Cost of one Einsum 'bgci, bgcj -> bgij' (Attention Matrix calculation)
    # Dimensions: (Batch, Groups, Channels_per_group, SeqLen) * (Batch, Groups, Channels_per_group, SeqLen)
    # Cost = b * g * c * i * j
    # Here i = j = seq_len
    
    flops_per_einsum = effective_batch * groups * group_planes * (seq_len * seq_len)
    
    # AxialAttention_dynamic performs 5 specific einsums:
    # 1. qr = einsum(q, q_embedding)
    # 2. kr = einsum(k, k_embedding)
    # 3. qk = einsum(q, k)
    # 4. sv = einsum(attn, v)
    # 5. sve = einsum(attn, v_embedding)
    
    total_einsum_flops = 5 * flops_per_einsum
    
    # Element-wise multiplications by factors (f_qr, f_kr, etc.)
    # These are negligible (O(N)) compared to O(N^2) einsums, but we can add them.
    # 4 multiplications on tensors of size (Batch, Groups, Seq, Seq) or (Batch, Groups, C, Seq)
    # We ignore them for GFLOPs as they are strictly memory bound, not compute bound.
    
    m.total_ops += torch.DoubleTensor([total_einsum_flops])

def count_axial_attention_wopos(m, x, y):
    """
    Custom FLOPs counter for AxialAttention_wopos.
    Captures the 2 expensive einsum operations: qk, sv (No positional embeddings)
    """
    inp = x[0]
    batch_size = inp.shape[0]
    ...
    # Standard einsum cost
    flops_per_einsum = effective_batch * groups * group_planes * (seq_len * seq_len)
    
    # AxialAttention_wopos performs 2 specific einsums:
    # 1. qk = einsum(q, k)
    # 2. sv = einsum(attn, v)
    
    total_einsum_flops = 2 * flops_per_einsum
    
    m.total_ops += torch.DoubleTensor([total_einsum_flops])

def count_axial_attention_standard(m, x, y):
    """
    Standard AxialAttention (if used). 
    Has same 5 einsums as dynamic, minus the scalar multiplications.
    """
    inp = x[0]
    batch_size = inp.shape[0]
    groups = m.groups
    ...
    if m.width:
        seq_len = inp.shape[3]; other_dim = inp.shape[2]
    else:
        seq_len = inp.shape[2]; other_dim = inp.shape[3]

    effective_batch = batch_size * other_dim
    flops_per_einsum = effective_batch * groups * group_planes * (seq_len * seq_len)
    
    # 5 einsums (qr, kr, qk, sv, sve)
    total_einsum_flops = 5 * flops_per_einsum
    m.total_ops += torch.DoubleTensor([total_einsum_flops])
...
if __name__ == "__main__":
    ...
    # Initialize Model
    print(f"Initializing MedT with img_size={img_size}...")
    model = MedT(imgDim=img_size, channels=img_chan, numclasses=num_classes)
    model.eval()

    # Create Dummy Input (Batch size 1)
    input_tensor = torch.randn(1, img_chan, img_size, img_size)

    # Register Custom Handlers
    # We map the Class Type to the Function defined above
    custom_ops = {
        AxialAttention_dynamic: count_axial_attention_dynamic,
        AxialAttention_wopos: count_axial_attention_wopos,
        AxialAttention: count_axial_attention_standard, # Included just in case
    }

    # Profile
    print("Profiling model (this traces the execution, including the patch-wise loops)...")
    macs, params = profile(model, inputs=(input_tensor, ), custom_ops=custom_ops, verbose=False)

    # Conversion
    # MACs = Multiply-Accumulate Operations
    # FLOPs approx 2 * MACs (1 mult + 1 add)
    gflops = (macs * 2) / 1e9
    total_params_million = params / 1e6

    print("-" * 40)
    print(f"MedT Architecture Analysis ({img_size}x{img_size} Input)")
    print("-" * 40)
    print(f"Total Parameters : {total_params_million:.3f} M")
    print(f"Total MACs       : {macs / 1e9:.3f} G")
    print(f"Total GFLOPs     : {gflops:.3f} G")
    print("-" * 40)
    print("Note: GFLOPs calculated as 2 * MACs. Includes manual counting of Attention einsums.")