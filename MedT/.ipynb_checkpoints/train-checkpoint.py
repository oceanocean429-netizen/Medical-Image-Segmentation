...
def main(config):
    ...
    for fold, (train_indices, val_indices) in enumerate(kfold.split(combined_dataset)):
        ...
        model = MedT(
            imgDim=256,
            channels=model_cfg['input_channels'], # model input channels (1 for gray-scale, 3 for RGB)
            numclasses=model_cfg['num_classes'],        # model output channels (number of classes in your dataset)
        )
...
if __name__ == '__main__':
    ...
    config.work_dir='./results/UltraLight_VM_UNet_PH2_Saturday_29_November_2025_09h_00m_20s'
    main(config)