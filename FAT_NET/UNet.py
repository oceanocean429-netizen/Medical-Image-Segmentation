...
class FAT_Net(nn.Module):
    def __init__(self, n_channels=3, n_classes=1):
        ...
        # 1. Setup Transformer with img_size=512
        ...
        # 2. CHANGED: Use stride=1 to preserve resolution (32x32)
        # We will handle resizing dynamically in forward()
        self.conv_seq_img = nn.Conv2d(in_channels=192, out_channels=512, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x):
        ...
        # 3. FIXED: Dynamic view based on input size
        # This prevents hardcoding errors if input size varies
        grid_h, grid_w = h // 16, w // 16
        feature_tf = feature_tf.view(b, 192, grid_h, grid_w)
        
        feature_tf = self.conv_seq_img(feature_tf)

        # 4. FIXED: Dynamic Interpolation
        # Forces Transformer output to match CNN output exactly before concatenation
