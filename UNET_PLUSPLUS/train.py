...
def main(config):
    ...
    for fold, (train_indices, val_indices) in enumerate(kfold.split(combined_dataset)):
        ...
        model = smp.UnetPlusPlus(
            ...
            in_channels=model_cfg['input_channels'], # model input channels (1 for gray-scale, 3 for RGB)
            classes=model_cfg['num_classes'],        # model output channels (number of classes in your dataset)
        )
