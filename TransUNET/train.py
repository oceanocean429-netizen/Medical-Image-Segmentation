...
def main(config):
    ...
    for fold, (train_indices, val_indices) in enumerate(kfold.split(combined_dataset)):
        ...
        model=TransUNet(
            class_num=model_cfg['num_classes'], 
            in_channels=model_cfg['input_channels'],
            img_dim=256,
            out_channels=8,
            head_num=4,       # Retained from example
            mlp_dim=512,      # Retained from example
            block_num=8,      # Retained from example
            patch_dim=16,     # Retained from example
        )
