...
class BCELoss(nn.Module):
    def __init__(self):
        ...
        # Use BCEWithLogitsLoss to accept raw logits from the model
...
class DiceLoss(nn.Module):
    ...
    def forward(self, pred, target):
        # 'pred' here is expected to be probabilities (after sigmoid)
        smooth = 1
        size = pred.size(0)
...
class BceDiceLoss(nn.Module):
    def __init__(self, wb=1, wd=1):
        ...
        # This now correctly uses BCEWithLogitsLoss
        self.bce = BCELoss()
        self.dice = DiceLoss()
        self.wb = wb
        self.wd = wd
    ...
    def forward(self, pred, target):
        # 'pred' is the raw logit output from your model
        
        # 1. Calculate BCE loss directly from logits
        bceloss = self.bce(pred, target)
        
        # 2. Calculate Dice loss by first applying sigmoid
